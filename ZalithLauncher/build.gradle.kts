...
val appId  = "com.denocompany.nexuslauncher"
...
android {
    ...
    buildTypes {
        ...
        create("proguard") { initWith(getByName("debug")); isMinifyEnabled = true; isShrinkResources = true }
        create("proguardNoDebug") { initWith(getByName("proguard")); isDebuggable = false }
        getByName("release") {
            isMinifyEnabled = false
    ...
    splits {
        ...
        if (arch != "all") {
            abi {
                isEnable = true; reset()
                when (arch) { "arm" -> include("armeabi-v7a"); "arm64" -> include("arm64-v8a"); "x86" -> include("x86"); "x86_64" -> include("x86_64") }
            }
        }
    }
    ...
    buildFeatures { prefab = true; buildConfig = true; viewBinding = true; compose = true }
...
fun generateJavaClass(sourceOutputDir: File, packageName: String, className: String, constantMap: Map<String, String>) {
    ...
    val constants = constantMap.entries.joinToString("\n") { (key, value) -> "\tpublic static final String $key = \"$value\";" }
    javaFile.writeText("""
        |/**
...
tasks.register("generateInfoDistributor") {
    doLast {
        val constantMap = mapOf("CURSEFORGE_API_KEY" to getCFApiKey(), "LAUNCHER_NAME" to project.property("launcher_name").toString(), "APP_NAME" to project.property("launcher_app_name").toString(), "BUILD_TYPE" to getBuildType())
...
dependencies {
    ...
    // Fase 4 — Jetpack Compose Navigation
    implementation("androidx.navigation:navigation-compose:2.7.7")
    // Fase 5 — Jetpack DataStore + Lifecycle Runtime Compose
